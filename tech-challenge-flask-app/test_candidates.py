...
try:
    with urllib.request.urlopen('http://<ALB_DNS_ENDPOINT>:80/gtg') as gtg:
        if gtg.code == 200:
...
try:
    with urllib.request.urlopen('http://<ALB_DNS_ENDPOINT>:80/candidate/John%20Smith', data=b'test') as insert:
        if insert.code == 200:
...
try:
    with urllib.request.urlopen('http://<ALB_DNS_ENDPOINT>:80/candidate/John%20Smith') as verify:
        if verify.code == 200:
...
try:
    with urllib.request.urlopen('http://<ALB_DNS_ENDPOINT>:80/candidates') as list:
        if list.code == 200:
